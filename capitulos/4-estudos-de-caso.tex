...
\begin{itemize}
    \item \textbf{Pesquisa Origem-Destino (OD)}: criamos as viagens de carro emuladas com base na pesquisa OD realizada pela Companhia de Metrô de São Paulo.
        ...
        Para gerar a carga para os experimentos da plataforma, emulamos o tráfego em São Paulo durante o horário de pico, das 5h40 às 8h40.
...
Foram executadas 20 iterações de cada uma das etapas, onde em cada iteração foram 10 minutos de emulação.
...
As três etapas desse experimento foram realizadas em dois \textit{laptops} com 8 CPUs virtuais (1.80 GHz) e 8 GB de memoria RAM.
...
\begin{enumerate}
    \item Executar uma instância em modo de produção da plataforma InterSCity em um \textit{laptops}.
    ...
    \item Realizar a emulação do cenário de tráfego de carros inteligente.

    \item Analizar os resultados obtidos ao final da emulação.
\end{enumerate}

A Figura \ref{fig:distancia_validacao}, apresenta o desvio padrão (no topo das barras) e a distância média percorrida pelos carros emulados nas três etapas deste experimento.
...
Uma análise semelhante foi realizada com a duração das viagens emuladas, como pode ser visto na Figura \ref{fig:duracao_validacao}.
O desvio padrão (no topo das barras) e a duração media das viagens emuladas nas três etapas do experimento são apresentados no gráfico.
...
o cenário com evento e sem PMV para informar os motoristas durasse 11 ciclos (2 )nsito pelo ciclo de vida + 8 arestas + 1 pelo recálculo da rota);
...
\begin{figure}[ht]
	...
	\caption{Média da duração das viagens emuladas no experimento.}
...
O modelo implementado no emulador correspondeu com o esperado e a comunicação entre as ferramentas foi validada, principalmente com o que diz respeito a atuação no ambiente emulado, sendo esse um
trabalho pioneiro.
...
carros inteligentes, mas dessa vez em uma escala maior e com dados abertos da cidade de São Paulo.
...
Foram executadas 20 iterações de cada uma das etapas, onde em cada iteração emulamos 1 hora e 30 minutos.
Neste experimento, utilizamos dados do OpenStreet Maps e a pesquisa Origem-Destino realizado pela companhia de metrô da cidade de São Paulo em 2007 para definição do cenário a ser emulado.
A seguir mais detalhes sobre esses dados.

não utilizamos o mapa viário completo da cidade de São Paulo, realizamos um recorte de um trecho da avenida Rebouças com um raio de 5km, próximo ao cruzamento da avenida Paulista.
...
Contudo, como não utilizamos o mapa completo da cidade, filtramos apenas as viagens cuja origem e destino pertencessem am recorte do mapa realizado.
...
\begin{itemize}
    ...
    \item \textbf{Pesquisa Origem-Destino (OD)}: criamos as viagens de carro emuladas com base na pesquisa OD realizada pela Companhia de Metrô de São Paulo.
        ...
        Emulamos o tráfego em São Paulo durante o horário das 8h às 9h30.
...
\begin{figure}[ht]
	...
	\caption{Posicionamento das PMVs no experimento de tráfego de carros inteligentes.}
	\label{fig:pos_pmvs}
\end{figure}
...
\begin{figure}[ht]
	...
	\caption{Local dos eventos de fechamento de via no experimento de tráfego de carros inteligentes.}
	\label{fig:eventos}
\end{figure}
...
Consideramos que, pelo fato das avenidas Rebouças e Paulista serem bem movimentadas, muitas das viagens emuladas neste experimento passassem por aquele trecho. 
...
As três etapas desse experimento foram realizadas novamente em dois \textit{laptops} com 8 CPUs virtuais (1.80 GHz) e 8 GB de memoria RAM.
...
A Figura \ref{fig:duracao_total}, apresenta o desvio padrão (no topo das barras) e a duração média das viagens de carro emuladas nas três etapas deste experimento.
No eixo Y, a duração é apresentada em segundos (ciclo de execução da emulação -- 1 ciclo = 1 segundo), e no eixo X os diferentes cenários utilizados em cada uma das 3 etapas.
...
Sendo essa uma possível melhoria no emulador, já que o mesmo ao criar o ator calcula e utiliza apenas um dos menores caminhos entre a origem e destino do carro, podendo essa rota coincidir entre
...
Avaliamos que alguns fatores podem ter influenciado nesse resultado, sendo a maioria deles voltado para os dados utilizados para a definicação do cenário de emulação deste experimento.
...
O conjunto de viagens emulados após recortar o mapa da cidade de São Paulo pode não ter sido representativa, a decisão de reduzir o mapa para facilitar a visualização do impacto do fechamento de vias
e PMVs pode não ter sido boa.
Além disso,  os resultados obtidos podem ter sudo influenciados devido a infraestrutura simples utilizada com apenas dois \textit{laptops}.
...
diferentes experimentos utilizando um ambiente emulado de uma cidade, podendo substituir \textit{testbeds} reais e aumentando a escala dos experimentos.
...
Inteligentes.
Já no segundo estudo de caso, demonstramos a viabilidade da atuação em um ambiente emulado, sendo esse um trabalho pioneiro na área de Cidades Inteligentes.
 
Contudo, como o foco principal do trabalho é prover uma solução de ambiente emulado para a realização de experimentos em plataformas de Cidade Inteligentes, acreditamos que algumas das análises
...
um ambiente de experimentação emulado deve possuir.
