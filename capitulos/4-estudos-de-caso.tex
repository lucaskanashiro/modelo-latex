...
E ao final, realizaremos uma análise crítica sobre os resultados obtidos, apontando possíveis melhorias a serem feitas.

\section{Ferramentas}

Neste trabalho integramos o simulador InterSCSimulator e a plataforma InterSCity para Cidades Inteligentes para a realização dos estudos de caso, por isso, ambos serão detalhados quanto a sua respectiva 
arquitetura na sequência.
Também será feita uma discussão de como os \textbf{requisitos fundamentais e de integração} apresentados no Capítulo \ref{cap:proposta} serão atendidos em cada ferramenta.
...
\textit{Data Processor}, oferece um serviço de processamento de dados em tempo real e históricos;
...
Neste estudo de caso, no contexto de estacionaento inteligentes, simulamos motoristas realizando seus respectivos percursos de carro e ao final procurando uma vaga disponível mais próxima ao
seu destino, provavelmente utilizando um aplicativo móvel que se comunica com a plataforma InterSCity.
Esse tipo de aplicativo poderia evitar alguns transtornos para os motoristas na árdua missão de estacionar seus carros no centro das grandes metrópoles, por exemplo.

A implementação feita deste cenário seguindo a arquitetura proposta apresentada no Capítulo \ref{cap:proposta}, bem como o experimento realizado, serão discutidos na seções seguintes.

\subsection{Implementação}
\label{sec:smart_parking}

Trazendo os conceitos de recursos da cidade e capacidades para este cenário, temos os carros e as vagas de estacionamento sendo recursos, onde os carros são capazes de
...
Por isso, realizamos um segundo estudo de caso, que será apresentado na seção seguinte, onde, por exemplo, a atuação no ambiente simulado se fez necessária.
...
Tanto que não foi necessária a implementação desse componente no segundo estudo de caso apresentado, tendo em vista que as ferramentas já atendiam os requisitos de integração.
